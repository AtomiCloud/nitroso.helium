...
class SearchCore {
  async mainKTMBPage(): Promise<MainPageToken> {
    ...
    if (from == null || to == null || token == null)
      throw new Error("Unable to find from or to station");
    ...
    const trainSchedules: TrainSchedule[] = elements
      .map((index, element) => {
        ...
        if (
          ...
          throw new Error(
            "Unable to find min_fare, available_seats, arrival_time, departure_time, train_service",
          );
...
export { SearchCore, MainPageToken, ProxyToken };
