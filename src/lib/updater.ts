...
class Updater {
  ...
  getAll(): Result<[TimingRes, TimingRes, RetrieveResult[]], ProblemDetails> {
    return this.utility
      .toResult(
        () => this.zinc.vTimingDetail("JToW", "1.0"),
        "Failed to get JToW train timings",
      )
      .andThen((j2w) =>
        this.utility
          .toResult(
            () => this.zinc.vTimingDetail("WToJ", "1.0"),
            "Failed to get WToJ train timings",
          )
          .andThen((w2j) =>
            this.checker
              .Check()
  ...
  async Update(): Promise<void> {
    ...
    const rr = r.andThen(
      async ([j2w, w2j, retrieved]): Promise<Result<void, ProblemDetails>> => {
        this.logger.info(
          { j2w, w2j, retrieved },
          "Retrieved existing schedules",
        );
